...
bool ToolManager::CheckPermission(const std::string& tool_name,
  ...
  if (!permission_.CheckPathOrAsk(path)) {
    *error = "Permission: operation on '" + path + "' denied by user";
    return false;
  }
  return true;
}
...
std::optional<ToolMessage> ToolManager::Execute(const std::string& name,
  ...
  if (!CheckPermission(name, arguments, error)) {
    return std::nullopt;
  }
